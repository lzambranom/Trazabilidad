...
@ aPosObj[1,1]+78 ,aPosObj[1,2]+10 MsGet oGetcEtiqueta  Var cEtiqueta			SIZE 200, 010 Valid (fVldEtq(@lOkEtq,@cEtiqueta,@cProduto,@cLote,@cInvima,@aWBrowse1,oWBrowse1,cNumero, cSerie, cClifor, cLoja, cNome, oDlg, lFiltra, dData, dDtaVal, cAutImp, _cMsgEtq, _cMsgInv,'cEtiqueta') )PIXEL Of oDlg PICTURE "@!" Font oFont2 Color CLR_BLUE When lOkEtq //!Empty(cEtiqueta) .and.
...
@ aPosObj[1,1]+75, aPosObj[1,2]+500 MsGet oGet10  Var cInvima					SIZE 100, 010 Valid(fVldLote(@lOkEtq,@cEtiqueta,@cProduto,@cLote,@cInvima,aWBrowse1,oWBrowse1,cNumero, cSerie, cClifor, cLoja, cNome, oDlg, lFiltra, dData, dDtaVal, cAutImp, _cMsgEtq, _cMsgInv)) PIXEL Of oDlg Font oFont2 F3 "SZ3" Color CLR_BLUE When Empty(cInvima)
...
if nOpcao = 1 //.and. !lFiltra
	...
	if Len(aWBrowse1) > 0
		...
		elseif Substr(cCombo1,1,1) $ "2/4" .and. Substr(cCombo2,1,1) = "4"  //Impressão das Etiquetas de Entrada quando Selecionado Pedido de Vebda ou OP SAIDA
			For i:= 1 to Len(aWBrowse1)
				nReg:= fBuscaEtq(Alltrim(aWBrowse1[i,2]),Alltrim(aWBrowse1[i,4]),Alltrim(aWBrowse1[i,11]))  // m&h
				if nReg > 0
					dbSelectArea("ZZH")
					dbGoto(nReg)
					If Empty(aWBrowse1[i][10])
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Static Function fVldEtq(lOkEtq,cEtiqueta,cProduto,cLote,cInvima,aWBrowse1,oWBrowse1,cNumero, cSerie, cClifor, cLoja, cNome, oDlg, lFiltra, dData, dDtaVal, cAutImp, _cMsgEtq, _cMsgInv,cFuncao)

Local lRet		:= .t.
Local lAchouPed	:= .t.
Local nTamEtq	:= Len(Alltrim(cEtiqueta)) +1
Local nPriPipe	:= At("|",cEtiqueta) //oFWChart:BSERIEACTION:={||ALERT("OI")}
local csbstetq  := SubStr(Alltrim(cEtiqueta),nPriPipe+1,nTamEtq) //M&H
Local nSegPipe	:= iif(At("|",csbstetq) > 0 , nPriPipe+At("|",csbstetq) , nTamEtq )
local csbttetq  := iif(nSegPipe == nTamEtq, '' , SubStr(Alltrim(cEtiqueta),nSegPipe+1,nTamEtq))//SubStr(Alltrim(cEtiqueta),nSegPipe,nTamEtq) //M&H
Local nTerPipe	:= iif(csbttetq == '',0, nTamEtq )//nTamEtq
Local nTamcId	:= Len(cEtiqueta)
Local cProduto	:= if(nPriPipe > 0, AllTrim(SubStr(cEtiqueta,1,nPriPipe-1)), Alltrim(cEtiqueta))
//Local cLote	:= if(nPriPipe > 0, AllTrim(SubStr(cEtiqueta,nPriPipe+1,nSegPipe)), space(Len(ZZH->ZZH_LOTE)) ) // m&h
Local cLote		:= if(nPriPipe > 0, AllTrim(SubStr(cEtiqueta,nPriPipe+1,(nSegPipe-nPriPipe)-1)), space(Len(ZZH->ZZH_LOTE)) ) // m&h
Local cNrEtq	:= Right(Alltrim(cEtiqueta),3)
Local nTamPrd	:= Len(SB1->B1_COD)-Len(cProduto)
Local nTamLot	:= Len(ZZH->ZZH_LOTE)-Len(cLote)
Local nTamInv	:= Len(ZZH->ZZH_INVIM1)-Len(cInvima) // M&H
Local nPos		:= 0
Local cMens		:= ""
Local lAchou	:= .f.
Local lVld		:= .f.
...
/*If Substr(cCombo1,1,1) = "2" // Pedido de Venda
		If Alltrim(cInvima) = ""   // M&H
			oDlg:SetNextFocus(oGet10)
		endif
endif	*/	
...
if nPipeInvi > 0  // M&H
	cInvima:= Substr(cInvima,1,nPipeInvi)
endif	*/

if Empty(cProduto)
	cProduto	:= space(Len(SB1->B1_COD))
	cLote		:= space(Len(ZZH->ZZH_LOTE))
	cInvima		:= space(Len(ZZH->ZZH_INVIM1)) // M&H
	cEtiqueta	:= Space(60)
	Return(.t.)
endif

///eet
if cFuncao $ 'cEtiqueta'  /////esta leyendo de la etiqueta
	if Substr(cCombo1,1,1) <> "1"
		cInvima	:= iif(nTerPipe > 0, AllTrim(SubStr(cEtiqueta,nSegPipe+1,nTerPipe)), space(Len(ZZH->ZZH_INVIM1)) ) // m&h
		if empty(alltrim(cInvima ))
			alert('Informar invima')	
			cEtiqueta		:= Space(60)		
			oGet8:Refresh()
			oGet9:Refresh()
			oGet10:Refresh()
...
if Substr(cCombo1,1,1) = "1" .or. Substr(cCombo1,1,1) = "3" 
	...
	if Alltrim(SB1->B1_TIPE) == "F" .and. Empty(dDtaVal)
		...
		cProduto	:= space(Len(SB1->B1_COD))
		cLote		:= space(Len(ZZH->ZZH_LOTE))
		cInvima		:= space(Len(ZZH->ZZH_INVIM1))
		oDlg:Refresh()
...
if Empty(cNumero)
	...
	cInvima := cInvima+space(nTamInv)  // M&H
	if !Empty(cProduto)	
		SB1->(dbSetOrder(1))
	...
	If lAchouProd
		...
		Elseif Substr(cCombo1,1,1) = "2" // Pedido de Venda
		    ...
		    /*If nPriPipe > 0 .and. nSegPipe > 0 .and. nTerPipe == 0   // M&H
				oDlg:SetNextFocus( oGet10 )
			endif */

			nRegZZH := fBuscaEtq(cProduto,cLote)  // M&H  colocar luego el invima
			If nRegZZH == 0
		...
		Elseif Substr(cCombo1,1,1) = "3" .or. Substr(cCombo1,1,1) = "4" // Ordem de Produção Entrada # SAIDA
			if Substr(cCombo1,1,1) = "3"///ENTRADA
				cProduto:= padr(Alltrim(cProduto),tamSx3("B1_COD")[1])
				cNumero := padr(Alltrim(cNumero),tamSx3("D3_OP")[1])
				SD3->(dbSetOrder(1))
				if SD3->(dbSeek(xFilial("SD3")+cNumero+cProduto))					
			...
			if lAchou
				if !Empty(cLote)
					...
					if Substr(cCombo1,1,1) $ "3"///ENTRADA
						...
						nRegZZH:= fBuscaEtq(cProduto,cLote,cInvima,cNumero)
		...
		if aWBrowse1[iTens,9] = 0
			...
			if !Empty(aWBrowse1[iTens,4])
				...
				elseif Substr(cCombo1,1,1) == "2" // Pedido de Venda
					...
					cInvima	:= padr(Alltrim(aWBrowse1[iTens,11]),tamSx3("ZZH_INVIM1")[1])
				...
				elseif Substr(cCombo1,1,1) = "4"  // Ordem de Produção Saida
					...
					nRecZZH:= fBuscaEtq(cProduto,cLote,cInvima,cNumero)	///op pasar invima	
				...
				ElseIf Substr(cCombo1,1,1) == "9" // Dev. Pedido de Venda
					...
					nRecZZH:= fBuscaEtq(cProduto,cLote,cInvima,cNumero)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Static Function fVldLote(lOkEtq,cEtiqueta,cProduto,cLote,cInvima,aWBrowse1,oWBrowse1,cNumero, cSerie, cClifor, cLoja, cNome, oDlg, lFiltra, dData, dDtaVal, cAutImp, _cMsgEtq, _cMsgInv)
...
if Empty(cLote)
	...
	if Alltrim(cLote) == Alltrim(Substr(cProduto,1,Len(ZZH->ZZH_LOTE))) 
		...
		if lPrdNotExist
			...
			cEtiqueta:= Alltrim(cProduto) + "|" + Alltrim(cLote)+ "|" + cInvima
			fVldEtq(lOkEtq,cEtiqueta,cProduto,cLote,cInvima,aWBrowse1,oWBrowse1,cNumero, cSerie, cClifor, cLoja, cNome, oDlg, lFiltra, dData, dDtaVal, cAutImp, _cMsgEtq, _cMsgInv)
			cProduto	:= space(Len(SB1->B1_COD))
			cLote		:= space(Len(ZZH->ZZH_LOTE))
			cInvima		:= space(Len(ZZH->ZZH_INVIM1)) // M&H
			cEtiqueta	:= Space(60)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Static Function fBuscaEtq(cProduto, cLote, cInvima,cxOp) // M&H
...
if  Substr(cCombo1,1,1) = "2" 		//Pedido de Venda 	//BUSCA QUE NO TENGA PEDIDO
	...
	//cQuery+= "     ZZH_INVIM1	= '" + cInvima + "'			AND "+CRLF //M&H
	cQuery+= "       ZZH_PEDIDO	= ''						AND "+CRLF
elseif  Substr(cCombo1,1,1) = "4"  	//Ordem de Produção Saida
	cQuery+= "       ZZH_COD	= '" + cProduto + "'		AND "+CRLF
	cQuery+= "       ZZH_LOTE	= '" + cLote + "'			AND "+CRLF
	cQuery+= "       ZZH_INVIM1	= '" + cInvima + "'			AND "+CRLF //M&H
	cQuery+= "       ZZH_OP 	= '" + cxOp + "'						AND "+CRLF
...
ElseIf  Substr(cCombo1,1,1) == "9"  // Dev. Pedido Venta	//BUSCA QUE TENGA SALIDA
	...
	cQuery+= "       ZZH_PEDIDO	= '" + cxOp + "'			AND "+CRLF
...
if Substr(cCombo1,1,1) $ "1/3/5/7" //.or.  Substr(cCombo1,1,1) = "3") //.and. !lFiltra
	For x:= 1 to Len(aWBrowse1)
		if aWBrowse1[x,1]
			...
			nReg := fBuscaEtq(Alltrim(aWBrowse1[x,2]),Alltrim(aWBrowse1[x,4]),Alltrim(aWBrowse1[x,11]))
			if nReg > 0
				dbSelectArea("ZZH")
				dbGoto(nReg)
...
oWBrowse1:AddColumn(TCColumn():New(cCampo19	,{||If(Len(aWBrowse1)>0,aWBrowse1[oWBrowse1:nAT, 12]," ")},PesqPict("ZZH","ZZH_INVIM1")		,,,"LEFT"	,160,.F.,.F.,,,,.F.,)) //M&H
