...
@ aPosObj[1,1]+73, aPosObj[1,2]+280 SAY oSay8 PROMPT cCampo7			   		SIZE 080, 010 PIXEL Of oDlg Font oFont2 Color CLR_BLUE 
@ aPosObj[1,1]+83 ,aPosObj[1,2]+280 MsGet oGet8  Var cProduto					SIZE 150, 010 PIXEL Of oDlg Font oFont2 Color CLR_BLUE When Empty(cProduto) //.and. !lFiltra  
@ aPosObj[1,1]+73, aPosObj[1,2]+440 SAY oSay8 PROMPT cCampo8	 			 	SIZE 080, 010 PIXEL Of oDlg Font oFont2 Color CLR_BLUE
@ aPosObj[1,1]+83, aPosObj[1,2]+440 MsGet oGet9  Var cLote						SIZE 150, 010 Valid(fVldLote(@lOkEtq,@cEtiqueta,@cProduto,@cLote,aWBrowse1,oWBrowse1,cNumero, cSerie, cClifor, cLoja, cNome, oDlg, lFiltra, dData, dDtaVal, cAutImp, _cMsgEtq, _cMsgInv)) PIXEL Of oDlg Font oFont2 Color CLR_BLUE When Empty(cLote) //.and. !lFiltra
	...
	If lAchouProd
			...
			if Substr(cCombo1,1,1) = "5"  //ENTRADA
				...
				if SD3->(dbSeek(xFilial("SD3")+cNumero+cProduto))
					While !SD3->(Eof()) .and. SD3->D3_DOC == cNumero .and. SD3->D3_COD == cProduto
						//if SD3->D3_CF == "DE0"  .and. Empty(SD3->D3_ESTORNO)
						if Substr(SD3->D3_CF,1,1) == "D"  .and. Empty(SD3->D3_ESTORNO)  // ALVARO HURTADO M&H	
							lAchou:= .t.
							exit
						endif
						SD3->(dbSkip())
				...
				if SD3->(dbSeek(xFilial("SD3")+cNumero+cProduto))
					While !SD3->(Eof()) .and. SD3->D3_DOC == cNumero .and. SD3->D3_COD == cProduto
						//if SD3->D3_CF == "RE0"  .and. Empty(SD3->D3_ESTORNO)
						if Substr(SD3->D3_CF,1,1) == "R"  .and. Empty(SD3->D3_ESTORNO) // ALVARO HURTADO M&H
							lAchou:= .t.
							exit							
						endif	
						SD3->(dbSkip())		
		...
		Elseif Substr(cCombo1,1,1) == "9" // Dev. Pedido de Venda
			...
			If nRegZZH == 0
				...
				cProduto	:= space(Len(SB1->B1_COD))
				cLote		:= space(Len(ZZH->ZZH_LOTE))
				cInvima     := space(Len(ZZH->ZZH_INVIM1))  // Alvaro Hurtado M&H
				cEtiqueta	:= Space(60)
				oDlg:Refresh()
				oGetcEtiqueta:Setfocus()
				oGetcEtiqueta:Refresh()
		...
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ fVldEtq / 10-Transferencia Interna		³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		...
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ fVldEtq / 11-Transferencia Interna		³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
...
Local nRecZZH		:= 0
Local cTipeB1		:= space(Len(SB1->B1_TIPE))
Local cInvima 		:= ""   //ALVARO HURTADO M&H
		...
		if aWBrowse1[iTens,9] = 0
			...
			if !Empty(aWBrowse1[iTens,4])
				...
				elseif Substr(cCombo1,1,1) = "2" // Pedido de Venda
					...
					cInvima	:= padr(Alltrim(aWBrowse1[iTens,4]),tamSx3("ZZH_INVIMA1")[1]) //ALVARO HURTADO M&H
					
					nRecZZH:= fBuscaEtq(cProduto,cLote)		
					
					if nRecZZH > 0
						If ValidaInvima(cInvima) //ALVARO HURTADO M&H
							dbSelectArea("ZZH")
							ZZH->(dbGoto(nRecZZH))
							RecLock("ZZH",.f.)
							...
							Msunlock()
						Else
							msgAlert("Invima Ivalido / Vencido !","A T E N C I Ó N")
						Endif	
					else
						msgAlert("No hay Saldo para Grabar Etiqueta !","A T E N C I Ó N")
					endif	
				...
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				//³ Grabando en tabla ZZH -A-Dev. Pedido	³
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
...
User Function ValidaInvima(Cod_Invima)    //ALVARO HURTADO M&H

	Local _lRet    := .T.
	//Local _cMsgInv := "Código INVIMA inválido o caducado."
	
	SZ3->(DBSetOrder(1)) // Indice 1 - Z3_FILIAL+Z3_COD
	//If SZ3->(DBSeek(xFilial('SZ3')+M->ZZH_INVIM1))
	If SZ3->(DBSeek(xFilial('SZ3')+Cod_Invima))
		If SZ3->Z3_VIGENCI < dDataBase // Caso a validade do INVIMA esteja vencida.
			//MsgAlert(_cMsgInv, 'Alerta')
			_lRet := .F.
		EndIf
